# JobSearch OS — Spec v1

> **Historical document.** This is the original design spec Fieldwork was built from
> ("JobSearch OS" was the working title), kept for the archaeology. Where it disagrees
> with the code, the code won. For current setup and usage, see [README.md](README.md).
...
4. **Fully customizable.** No user fact, threshold, phrase, or rule in code. Everything in
   `settings`/`profile` with a UI. Olivia is user #1, not the default values.
...
7. **Whimsy, gently.** Warm microcopy and small delights; never cute about rejection.
...
| `settings` | key, value jsonb — timing rules (nudge_days=5–7, ghost_weeks=3, thankyou_hours=24), theme, whimsy_level, per-action model map, board columns |
| `applications` | company, title, status (enum: to_apply, applied, phone_screen, interviewing, final_round, offer, accepted, rejected, withdrawn, ghosted), date_applied, verdict (yes/soft_yes/soft_no/no), comp_posted, comp_min, comp_max, remote_type, source, next_action, next_action_due, resume_filename, cover_letter bool |
...
1. **Today** (home) — the action queue. Everything due, one card each, one button each.
2. **Pipeline** — kanban by status, drag to move (writes a status_change event), filters,
   aging badges, NEXT-due indicators.
3. **Intake** — paste JD text or URL. Liveness check → scorecard → verdict card → file it.
...
6. **Insights** — funnel chart, deaths-by-stage, rejection reasons clustered, title
   win-rates, weekly velocity. The lessons log rendered as evidence.
...
8. **Onboarding wizard** (product-shaped requirement) — the resume-builder setup interview
   as screens: record builder, rules, hooks, titles. Olivia's path: "import existing."
...
| "sweep my inbox" | **[Sweep now]** (Phase 4 server-side; Phase 1–3 shows last sweep result from the shared DB) |
| "mark it ghosted" | Auto-suggested by timing rules; one-click confirm |
...
- `scorecard(jd_text|url)` — liveness check, extract comp/location/reqs, verdict per the
...
1. Import script: 146 xlsx rows + jds\ + contacts.md + lessons-learned.md → Supabase
   (one-time, idempotent, dry-run mode first).
...
|---|---|---|
| 0 — Kit | This spec locked; Supabase project + schema + RLS; import dry-run report | Olivia locks spec; import numbers match tracker |
...
## 11. Open questions for Olivia's review
...
5. Name check: "JobSearch OS" — keep, or does it want a Keiter-style name?
