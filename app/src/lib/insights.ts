...
import type { FwApplication, FwEvent, FwLesson, FwStatus } from './types';
...
export async function loadInsightsData(): Promise<InsightsData> {
  const [appsRes, lessonsRes, eventsRes] = await Promise.all([
...
export type GroupBy = 'source' | 'title' | 'remote_type' | 'verdict' | 'cover_letter';

export const GROUP_BYS: { id: GroupBy; label: string }[] = [
  { id: 'source', label: 'Source' },
  { id: 'title', label: 'Title' },
  { id: 'remote_type', label: 'Remote type' },
  { id: 'verdict', label: 'Fit verdict' },
...
function groupKey(app: FwApplication, groupBy: GroupBy): { key: string; label: string } {
  switch (groupBy) {
    ...
    case 'verdict': {
      const v = app.verdict ?? '';
      return { key: v || '—', label: v ? VERDICT_LABELS[v] ?? v : 'Not scored' };
    }
    case 'cover_letter':
