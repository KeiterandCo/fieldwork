...
/** Applies `fw_profile.file_name_pattern` (e.g. "{FirstName}_{LastName}_{company}_{title}") to a
...
/** Records that a resume was exported for this application: sets resume_filename, persists the
 * final resume JSON, and logs a timeline event that carries the full readable resume text as a
 * snapshot. `fw_event_type` has no dedicated "resume built" value, so this uses the closest
 * existing one ('note'); the RESUME_EVENT_PREFIX lets the timeline and History treat it
