...
/** Staged labels for the draft loading bar, mirroring the resume builder's progress feel. */
const DRAFT_STAGES = [
  'Reading your career record…',
  'Pulling in the role and context…',
  'Writing your draft…',
];

/** Playful rotating lines for the draft bar (module-level so the rotation timer is stable). */
const DRAFT_PHRASES = [
  'Reading your career record…',
  'Finding the honest hook…',
  'Warming up the tone…',
  'Keeping it human…',
  'Cutting the filler…',
  'Landing the close, gently…',
      ...
      <div className="flex w-full max-w-xl flex-col gap-4 rounded-xl border border-border bg-surface p-6 shadow-xl">
        <div className="flex items-start justify-between">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg px-2 py-1 text-sm text-text-dim transition-colors hover:text-text"
          >
            ✕
          </button>
        </div>
        ...
        {state === 'generating' && (
          <div className="py-2">
            <BuildProgress stages={DRAFT_STAGES} phrases={DRAFT_PHRASES} />
          </div>
        )}
